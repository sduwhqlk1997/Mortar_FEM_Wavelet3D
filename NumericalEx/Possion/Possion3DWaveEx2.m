...
J=7;
j0=2;
jcoef=[1;1;1]; % j1coef*j1+j2coef*j2+j3coef*j3<=J
[~,jcoef_min]=min(jcoef);
jcoef_other=[1;2;3];
...
[DofIndex_sparse,Table_sparse] = GenWaveletSparseInfo3D(j0,J,...
    BaseFunIndex_ref,BaseFunIndex_ref,BaseFunIndex_ref,jcoef);
% 生成三维参考基本矩阵
...
u_num=sortrows([[index_I,u_I];[boundary_index,zeros(length(boundary_index),1)]]);
u_num = u_num(:,2);
% 生成数值解
