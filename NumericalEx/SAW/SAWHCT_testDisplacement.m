...
[K_IDT,Dof_Index_IDT] = IDT_Schur(K_IDT,Dof_Index_IDT,interval_sub,interval_ele,false);
...
F=zeros(size(K_IDT,1),1);
F=F-sum(K_IDT(:,index_e),2);
K_IDT(index_e,:)=[];
K_IDT(:,index_e)=[];
F(index_e,:)=[];
Dof_Index_IDT(index_e,:)=[];
Sol=K_IDT\F;
% 测试左边界位移
load('D:\Code\M\Mortar_FEM_Wavelet\NumericalEx\SAW\Data\FEMLboundSol.mat')
[i,j]=find(abs(Dof_Index_IDT(:,2)-Dof_Index_L(:,3).')/b1<1e-10&...
    abs(Dof_Index_IDT(:,3)-Dof_Index_L(:,4).')/c1<1e-10&...
    Dof_Index_IDT(:,4)==Dof_Index_L(:,1).'&...
    Dof_Index_IDT(:,1)==-1&...
    Dof_Index_IDT(:,4)==3);
err=norm(Sol(i)-Sol_L(j),'inf')/norm(Sol_L(j),'inf')