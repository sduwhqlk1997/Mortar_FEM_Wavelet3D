function viewMesh(node,elem)
%VIEWMESH 此处显示有关此函数的摘要
%   此处显示详细说明
...
if ~iscell(elem) && dim==3 && ~isexpr
    elemcut = elem;
    totalFace = [elemcut(:,[1,2,4,3]); elemcut(:,[5,6,8,7]); elemcut(:,[1,2,6,5]); elemcut(:,[3,4,8,7]); elemcut(:,[1,3,7,5]);elemcut(:,[2,4,8,6])];
    %totalFace = sort(totalFace,2);
    face = unique(totalFace,'rows');  % triangles
    h = patch('Faces', face, 'Vertices', node);
%     h = patch('Faces', face, 'Vertices', node,'FaceColor','none');
end
...
% set(h,'facecolor','b','edgecolor','k','LineWidth',1);
